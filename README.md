...
Hele ordlisten og fasiten for begge retninger føres i [`data/vocabulary-canonical.json`](data/vocabulary-canonical.json). Rediger `norsk`, `spansk`, `kategori` eller svarlistene `svar.es-no`/`svar.no-es` direkte. Kjør deretter `npm run audit:apply -- --apply --replace` for å oppdatere `index.html`. `--replace` gjør at canonical-fila er hele fasiten (gamle svar fjernes); uten `--replace` beholdes eksisterende svar og audit-godkjente kandidater legges til. Etter påføring kjøres `npm run build:app` og `npm run test:all`. `data/vocabulary-canonical-review.json` er en eventuell arbeidskopi for gjennomgang og er ikke aktiv før den er synkronisert til `vocabulary-canonical.json` og påført.

Innholdet i `vocabulary-canonical.json` er lærergodkjent 2026-09-09 (fra `vocabulary-canonical-review.json`): blant annet rettelse av «allehelgensdagen» til «el Día de Todos los Santos», «loft» til «el ático», personlig «a» i «visitar a mi abuela», ny kategori «Mat og drikke», og fjerning av de 15 verbbøying-kortene (verbøying øves via verbmodulen).
